...
// Import company data
const companyData = JSON.parse(fs.readFileSync(path.join(__dirname, 'src/data/companyData.js'), 'utf8').replace('export default ', '').replace(/;$/, ''));
...
const generateSitemapXML = () => {
  ...
  const productPages = companyData.products.map((product) => ({
  ...
  const blogPages = companyData.blogs?.map((blog) => ({
  ...
  const urls = [...staticPages, ...productPages, ...blogPages];
